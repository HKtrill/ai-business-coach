"""
The solver: scan every realisable band, keep the best feasible one.

Train/OOF only
--------------
No method here accepts test probabilities or test labels, and none should ever
be given them. That is the black-box counterpart of GLASS selecting its rules
against ``y_val`` inside ``fit`` and never looking at the holdout.
"""
...
class ThresholdSolver:
    ...
    # ------------------------------------------------------------------
    # Pass 1 — route NOT_SUBSCRIBE when p < t1
    # ------------------------------------------------------------------
    def solve_pass1(
        self,
        proba: np.ndarray,
        ...
        Largest means maximum Pass 1 coverage at the declared error budget,
        which is the role GLASS Pass 1 plays: route away as much of the negative
        mass as the leakage budget allows.

        ``proba`` and ``y`` must come from out-of-fold predictions on the
        training population.
        """
        p, y = prepare_scores(proba, y, "Pass 1")
        c = self.constraints
        n = len(p)
        total_pos = int(y.sum())
        ...
        for k in lower_band_sizes(p_s):
            ...
            rows.append({
                ...
                "threshold": upper_edge(p_s, k),
                "feasible": not why,
                "binding": ",".join(why),
            })
            if not why:
                best_k = k  # sizes ascend -> the last feasible band is the largest

        sweep = pd.DataFrame(rows) if keep_sweep else None

        if best_k is None:
            return ThresholdResult.infeasible(
                "Pass 1",
                reason=(
        ...
        pos = int(cum_pos[best_k])
        best = ThresholdResult(
            threshold=float(upper_edge(p_s, best_k)),
            feasible=True,
            reason="ok",
            band_size=int(best_k),
            band_fraction=float(best_k / n),
            band_precision=float((best_k - pos) / best_k),
    ...
    # ------------------------------------------------------------------
    # Pass 2 — flag SUBSCRIBE when p > t2
    # ------------------------------------------------------------------
    def solve_pass2(
        self,
        proba: np.ndarray,
        ...
        Smallest means maximum recall at the declared precision, which is the
        role GLASS Pass 2 plays.
        ...
        n_positives_global
            ...
            ``constraints.recall_denominator == "global"`` — the GLASS-comparable
            setting, because GLASS Pass 2 rules are scored by ``RuleEvaluator``
            against every positive in the evaluation set, not only those the
            router left unrouted.
        """
        p, y = prepare_scores(proba, y, "Pass 2")
        c = self.constraints
        n = len(p)
        ...
        for m in upper_band_sizes(p_s):
            ...
            rows.append({
                ...
                "positives_in_band": pos,
                "recall": rec,
                "threshold": lower_edge(p_s, m),
                "feasible": not why,
                "binding": ",".join(why),
            })
            if not why:
                # sizes ascend -> the last feasible band is the largest, and the
                # largest band is the smallest threshold.
                best_m = m
        ...
        pos = int(suffix_pos[best_m])
        best = ThresholdResult(
            threshold=float(lower_edge(p_s, best_m)),
            feasible=True,
            reason="ok",
            band_size=int(best_m),
            band_fraction=float(best_m / n),
            band_precision=float(pos / best_m),
            positives_in_band=pos,
            leakage_rate=0.0,
    ...
    def make_pair(
        ...
        cascade is structurally incapable of overlapping bands, so ``RFRouter``
        skips this check and ``SingleRFRouter`` applies it.
        """
        pass1.raise_if_infeasible()
        pass2.raise_if_infeasible()

        if (self.constraints.require_band_ordering
                and pass1.threshold > pass2.threshold):
            raise ValueError(
