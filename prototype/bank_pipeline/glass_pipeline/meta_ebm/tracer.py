...
(GLASS LR + GLASS Router + GLASS EBM) on the test split.

GLASS Router rows it does not cover (abstain) count as a 0 prediction here,
as before — this is a descriptive trace, not the arbiter's view.
...
def run_cascade_trace(
    ...
    if verbose:
        ...
        print("=" * 80)
        print("   LR + EBM + GLASS Router — real model agreement on test set")
        print("=" * 80)
    ...
    fig.suptitle(
        "Venn Diagram Trace — LR + EBM + GLASS Router\n(UCI Bank Marketing — Real Cascade)",
