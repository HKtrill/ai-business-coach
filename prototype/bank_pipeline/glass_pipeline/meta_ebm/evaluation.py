...
=====================================
Evaluation metrics and disagreement analysis for the three-model ensemble.

The GLASS Router is partial-coverage — disagreement against it is computed
only over the rows where it votes (Pass 1 | Pass 2).
"""
...
def analyze_disagreements(
    ...
    Parameters
    ----------
    y_true             : true labels (train set; predictions are OOF)
    lr_pred            : LR binary predictions
    ebm_pred           : EBM binary predictions
    glass_pred         : GLASS Router binary predictions (0 for Pass 1, 1 for Pass 2)
