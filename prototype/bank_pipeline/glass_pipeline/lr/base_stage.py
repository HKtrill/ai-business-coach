...
class BaseLRStage(ABC):
    ...
    def evaluate(
        ...
        -------
        metrics : dict
            The shared metric set (``shared.metrics.compute_metrics``) at a
            fixed 0.5 threshold, uncalibrated: accuracy, precision, recall,
            f1, f2, roc_auc, pr_auc, brier, ece, pred_pos_rate.

        Notes
        -----
        PR 33: previously accuracy/precision/recall/f1/roc_auc only, which left
        the baseline off the F2 axis every other number in the cascade is on.
        """
        ...
        print("\n" + "=" * 80)
        print("📊 EVALUATING LR STAGE ON TEST SET (threshold=0.50, uncalibrated)")
        print("=" * 80)

        y_pred  = self.predict(X_test)
        y_proba = self.predict_proba(X_test)

        self.metrics = compute_metrics(y_test, y_pred, y_proba, 0.5, "none")
