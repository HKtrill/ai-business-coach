"""
glass_router.rf.rf_training
===========================
RF training for the Stage 2 two-pass Random Forest router (black-box
comparator to GLASS Router).
...
def _select_oof_threshold(
    ...
    """Select the F2-maximizing threshold from pooled OOF probabilities."""
    ...
    scores = np.array([
        fbeta_score(
            y,
            (oof_proba >= threshold).astype(int),
            beta=2,
            zero_division=0,
        )
...
def train_rf_stage(
    ...
    metrics_cv = _evaluate_rf(
        params, X_train_eng, y_train, n_eval_folds, random_state
    )
    ...
    # ── 3. Select operating threshold from pooled OOF predictions ─────────────
    print(f"\n{'─' * 80}")
    print("🎯 Selecting operating threshold from OOF predictions...")
    print(f"{'─' * 80}")

    threshold, oof_f2 = _select_oof_threshold(
        params,
        X_train_eng,
        y_train,
        n_eval_folds,
    ...
    print(f"   OOF threshold : {threshold:.4f}")
    print(f"   OOF F2        : {oof_f2:.4f}")


    # ── 4. Final refit on full X_train ────────────────────────────────────────
    ...
    return RFResult(
        ...
        params=params,
        threshold=threshold,
        threshold_source="pooled OOF F2",
