"""
glass_router.rf.artifacts
=========================
Persist RF Stage 2 results to disk.

Saves rf_result (RFResult dataclass) and GLASS_ROUTER_DATA (engineered
feature dict) as a single joblib artifact. GlassRouterPipeline reads
rf_result.model from the loaded artifact.
...
---------------------
    ...
    glass_router_data  GLASS_ROUTER_DATA dict (X_eng_train, y_eng_train,
                       X_eng_test, y_eng_test, feature_names)
                       [renamed from legacy key 'brw_data']
    training_date      ISO timestamp string

Public API
----------
save_rf_artifact(rf_result, glass_router_data, output_dir) → dict
"""
...
def save_rf_artifact(
    rf_result:  RFResult,
    glass_router_data: dict,
    output_dir: str = "./models/rf",
) -> dict:
    """
    Persist rf_result and glass_router_data to a timestamped joblib file.
    ...
    print(f"   Features : {glass_router_data['X_eng_train'].shape[1]} binary bins")
    print(f"   Train    : {glass_router_data['X_eng_train'].shape[0]:,} samples")
    print(f"   Test     : {glass_router_data['X_eng_test'].shape[0]:,} samples")
    ...
    artifact = {
        ...
        # ── Data contract for GLASS Router ────────────────────────────────
        "glass_router_data":  glass_router_data,
