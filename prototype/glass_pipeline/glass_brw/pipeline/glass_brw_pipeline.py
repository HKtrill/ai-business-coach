...
from glass_brw.core.rule import SUBSCRIBE, NOT_SUBSCRIBE, ABSTAIN
...
class GLASSBRWPipeline:
    ...
    def __init__(self, config, rf_model=None, segment_builder=None, rule_logger=None):
        ...
        self.rule_evaluator = RuleEvaluator(
            segment_builder=self.segment_builder,
            min_support=min(config.min_support_pass1, config.min_support_pass2),
        )
        ...
        self.ilp_selector = ILPRuleSelector(
            ...
            lambda_rf_uncertainty=config.lambda_rf_uncertainty,
            lambda_rf_misalignment=config.lambda_rf_misalignment,
        )
    ...
    def predict_proba(self, X, base_rate=None):
        ...
        Args:
            X: Features DataFrame
            base_rate: Prior probability of SUBSCRIBE. If None, uses training rate.
        ...
        for i in range(n):
            ...
            # abstains keep the base-rate prior
