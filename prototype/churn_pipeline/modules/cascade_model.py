...
class CascadeModel:
    ...
    Stage 1: Lasso Logistic Regression (feature selection + interpretability)
    ...
    def _create_balanced_sampling_pipeline(self):
        # BALANCED: Moderate oversampling for precision-recall balance
        smote = BorderlineSMOTE(random_state=self.random_state, sampling_strategy=0.6)  # Reduced from 0.7 for better precision
        undersample = RandomUnderSampler(random_state=self.random_state, sampling_strategy=0.82)  # Slight increase from 0.8
    ...
    def train_cascade_pipeline(self, X_train_preprocessed, y_train, X_test_preprocessed, y_test):
        ...
        print("\nTraining Stage 1: Lasso Logistic Regression...")
        self.stage1_model = LogisticRegression(
            penalty='l1',
            C=0.3,  # BALANCED: Moderate regularization for better precision-recall balance (from 0.5)
            solver='liblinear',  # Best solver for L1 penalty
        ...
        non_zero_coefs = np.sum(self.stage1_model.coef_[0] != 0)
        total_features = len(self.stage1_model.coef_[0])
        print(f"  Lasso selected {non_zero_coefs}/{total_features} features")
        ...
        # Weighted ensemble (BALANCED: Adjusted weights for precision-recall stability)
        y_proba_final = y_proba1*0.4 + y_proba2*0.3 + y_proba3*0.3  # More weight back to conservative Lasso
        threshold = 0.5  # BALANCED: Moderate threshold for precision-recall balance (from 0.4)
    ...
    def get_feature_importance(self):
        """
        Returns Lasso coefficients as feature importance measure.
        Positive values indicate higher churn probability.
        """
        if self.stage1_model is not None:
            return np.abs(self.stage1_model.coef_[0])  # Absolute values for importance ranking
        return None
    
    def get_lasso_coefficients(self):
        """
        Returns raw Lasso coefficients with their signs for interpretability.
        """
        if self.stage1_model is not None:
            return self.stage1_model.coef_[0]
        return None